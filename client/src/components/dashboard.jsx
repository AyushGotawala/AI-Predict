...
const initialPredictions = [
  ...
  { type: "🖼️ Image Content Analysis", time: "12 minutes ago", status: "inappropriate" },
...
const Dashboard = () => {
  ...
  const handleUploadImage = () => {
    setNotification({ message: "Image uploaded successfully! Analyzing...", type: "success" });
    setTimeout(() => {
      addPrediction("🖼️ Image Content Analysis", "Just now", "safe");
      setNotification({ message: "Analysis complete: Image is safe!", type: "success" });
    }, 2000);
  };
  ...
  const handleCheckEmail = () => {
    ...
    if (email && email.trim()) {
      setNotification({ message: "Email submitted for analysis...", type: "info" });
      setTimeout(() => {
    ...
    <div className={styles["dashboard-container"]}>
      ...
      <main className={styles["main-content"]}>
        <header className={`${styles.header} ${styles["fade-in"]}`}>
          <div>
            <h1 className={styles["welcome-text"]}>Welcome back, Alex!</h1>
            <p className={styles["date-text"]} id="currentDate">
              {currentDate}
            </p>
          </div>
          <div className={styles["user-info"]}>
            <button className={styles["notification-btn"]} onClick={handleNotificationClick}>
              🔔
            </button>
        ...
        <div className={`${styles["stats-grid"]} ${styles["fade-in"]}`}>
          <div className={styles["stat-card"]}>
            ...
            <div className={styles["stat-value"]}>{totalPredictions.toLocaleString()}</div>
            <div className={styles["stat-label"]}>Total Predictions</div>
            <div className={`${styles["stat-change"]} ${styles.positive}`}>+12% from last week</div>
          </div>
          <div className={styles["stat-card"]}>
            ...
            <div className={`${styles["stat-change"]} ${styles.positive}`}>+8% from last week</div>
          </div>
          <div className={styles["stat-card"]}>
            <div className={styles["stat-icon"]}>📧</div>
            <div className={styles["stat-value"]}>1,554</div>
            <div className={styles["stat-label"]}>Emails Scanned</div>
            <div className={`${styles["stat-change"]} ${styles.positive}`}>+15% from last week</div>
          </div>
          <div className={styles["stat-card"]}>
            <div className={styles["stat-icon"]}>🛡️</div>
            <div className={styles["stat-value"]}>98.7%</div>
            <div className={styles["stat-label"]}>Accuracy Rate</div>
            <div className={`${styles["stat-change"]} ${styles.positive}`}>+0.3% from last week</div>
          </div>
        </div>
        ...
        <div className={styles["content-grid"]}>
            ...
            <ul className={styles["prediction-list"]}>
              {predictions.map((pred, idx) => (
                <li className={styles["prediction-item"]} key={idx}>
                  ...
                  <span className={`${styles["prediction-status"]} ${styles[`status-${pred.status}`]}`}>
          ...
          <div className={`${styles["content-card"]} ${styles["fade-in"]}`}>
            ...
            <div className={styles["upload-section"]} onClick={handleUploadImage} style={{ cursor: "pointer" }}>
              <div className={styles["upload-icon"]}>📁</div>
              <div className={styles["upload-text"]}>Upload Image for Analysis</div>
              <button className={styles["upload-btn"]}>Choose File</button>
            </div>
            <div className={styles["upload-section"]} onClick={handleCheckEmail} style={{ cursor: "pointer" }}>
              <div className={styles["upload-icon"]}>📧</div>
